-- #########################################################################
-- Module Name : dwkit.commands.dwactions
-- Owner       : Commands
-- Version     : v2026-01-25A
-- Purpose     :
--   - Handler for "dwactions" command surface.
--   - SAFE: prints ActionModelService snapshot/state.
...
local function _fallbackErr(msg)
  _fallbackOut("[DWKit Actions] ERROR: " .. tostring(msg))
end
...
function M.dispatch(ctx, kit)
  ...
  local svc = K.services.actionModelService
  if type(svc) ~= "table" then
    C.err("DWKit.services.actionModelService not available. Run loader.init() first.")
    return false, "actionModelService missing"
  end

  return _printServiceSnapshot(C, "ActionModelService", svc), nil
end
