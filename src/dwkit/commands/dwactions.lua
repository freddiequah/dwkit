...
--       * dwactions
--   - Prints ActionModelService snapshot (best-effort, SAFE).
--
-- Public API  :
...
function M.dispatch(ctx, kit)
  ...
  _out(ctx, "[DWKit Service] ActionModelService")
  local svc = _getService(kit, "actionModelService")
  if not svc then
    _err(ctx, "DWKit.services.actionModelService not available. Run loader.init() first.")
    return
  end
