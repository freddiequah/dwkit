...
--       * dwskills
--   - Prints SkillRegistryService snapshot (best-effort, SAFE).
--
-- Public API  :
...
function M.dispatch(ctx, kit)
  ...
  _out(ctx, "[DWKit Service] SkillRegistryService")
  local svc = _getService(kit, "skillRegistryService")
  if not svc then
    _err(ctx, "DWKit.services.skillRegistryService not available. Run loader.init() first.")
    return
  end
