-- #########################################################################
-- Module Name : dwkit.commands.dweventlog
-- Owner       : Commands
-- Version     : v2026-01-27A
-- Purpose     :
--   - Split SAFE command: dweventlog [n]
--   - Delegates to dwkit.commands.event_diag.printLog
...
function M.dispatch(...)
    ...
    if _isArrayLike(a1) and tostring(a1[1] or "") == "dweventlog" then
        ctx = nil
        kit = nil
        tokens = a1
    else
        ctx = a1
        if _isArrayLike(a2) and tostring(a2[1] or "") == "dweventlog" then
            kit = nil
            tokens = a2
        elseif _isArrayLike(a3) and tostring(a3[1] or "") == "dweventlog" then
            kit = a2
            tokens = a3
        else
            return false
    ...
    local n = tostring((tokens and tokens[2]) or "")
    ...
    if not okED or type(ED) ~= "table" or type(ED.printLog) ~= "function" then
        D.err("dwkit.commands.event_diag.printLog not available (" .. tostring(errED) .. ")")
        return true
    end

    local okCall, errOrNil = pcall(ED.printLog, D, st, n)
    if not okCall then
        D.err("event_diag.printLog threw error: " .. tostring(errOrNil))
    end
