-- #########################################################################
-- Module Name : dwkit.commands.dweventsub
-- Owner       : Commands
-- Version     : v2026-01-27A
-- Purpose     :
--   - Split SAFE command: dweventsub <EventName>
--   - Delegates to dwkit.commands.event_diag.subOn
--   - State: dwkit.services.event_diag_state
--
-- Public API:
--   - dispatch(ctx, kit, tokens) -> boolean
...
function M.dispatch(...)
    ...
    if _isArrayLike(a1) and tostring(a1[1] or "") == "dweventsub" then
        ctx = nil
        kit = nil
        tokens = a1
    else
        ctx = a1
        if _isArrayLike(a2) and tostring(a2[1] or "") == "dweventsub" then
            kit = nil
            tokens = a2
        elseif _isArrayLike(a3) and tostring(a3[1] or "") == "dweventsub" then
            kit = a2
            tokens = a3
        else
            return false
    ...
    if evName == "" then
        D.err("Usage: dweventsub <EventName>")
        return true
    end
    ...
    if not okED or type(ED) ~= "table" or type(ED.subOn) ~= "function" then
        D.err("dwkit.commands.event_diag.subOn not available (" .. tostring(errED) .. ")")
        return true
    end

    local okCall, errOrNil = pcall(ED.subOn, D, st, evName)
    if not okCall then
        D.err("event_diag.subOn threw error: " .. tostring(errOrNil))
    end
